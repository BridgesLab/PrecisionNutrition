...
Table: Baseline LDL-C distribution across analytic cohort

|Metric                 |   Value|
|:----------------------|-------:|
|N patients             | 20015.0|
|Min baseline LDL-C     |     3.0|
|25th percentile        |    85.0|
|Median baseline LDL-C  |   110.0|
|Mean baseline LDL-C    |   112.8|
|75th percentile        |   137.0|
|Max baseline LDL-C     |   380.0|
|Missing baseline LDL-C |     0.0|
...
Table: Baseline LDL-C by case/control status

|Group             |     N|  Mean| Median|   SD|
|:-----------------|-----:|-----:|------:|----:|
|Controls (MACE=0) | 17144| 114.1|    112| 38.3|
|Cases (MACE=1)    |  2871| 105.4|    100| 42.1|
...
::: {.cell-output-display}
![](figures-baseline/baseline-ldlc-characterisation-1.png){width=2100}
:::
:::
...
Table: Baseline LDL-C and MACE rates by baseline statin status

|Statin status            |     N| Mean LDL-C (mg/dL)| Median LDL-C (mg/dL)| MACE events| MACE rate (%)|
|:------------------------|-----:|------------------:|--------------------:|-----------:|-------------:|
|Statin-naive at baseline | 19227|              113.2|                  111|        2767|          14.4|
|On statin at baseline    |   788|              104.0|                   98|         104|          13.2|
...
Table: Statin intensity distribution among baseline statin users

|baseline_intensity |   n|
|:------------------|---:|
|low                |  61|
|moderate           | 495|
|high               | 226|
...
Table: Comorbidity prevalence at baseline (time zero)

|Condition     | N at baseline| % of cohort|
|:-------------|-------------:|-----------:|
|Diabetes      |          3217|        16.1|
|Hypertension  |          6293|        31.4|
|Obesity       |          3286|        16.4|
|Renal failure |           795|         4.0|
...
Table: tmerge dataset structure
...
|Patients                           | 20007|
|MACE events                        |  2869|
|Patients with >1 event (must be 0) |     0|
...
Table: Covariate coverage across intervals

|Covariate                      | % of intervals|
|:------------------------------|--------------:|
|% intervals with diabetes Dx   |           16.1|
|% intervals with HTN Dx        |           31.4|
|% intervals statin at baseline |            3.9|
...
Table: MACE events per stratum

|period | baseline_statin|age_cat |cardiomet |  n|
|:------|---------------:|:-------|:---------|--:|
|0-5yr  |               1|<45     |HTN only  |  1|
|0-5yr  |               1|<45     |DM+HTN    |  1|
|0-5yr  |               1|45-54   |Neither   |  1|
|0-5yr  |               1|45-54   |DM only   |  1|
|0-5yr  |               1|<45     |Neither   |  2|
|0-5yr  |               1|55-64   |Neither   |  2|
|0-5yr  |               1|65-74   |Neither   |  2|
|0-5yr  |               1|75+     |Neither   |  2|
|0-5yr  |               1|75+     |DM only   |  2|
|0-5yr  |               1|45-54   |HTN only  |  3|
|0-5yr  |               1|55-64   |DM only   |  3|
|0-5yr  |               1|45-54   |DM+HTN    |  9|
|0-5yr  |               1|55-64   |HTN only  | 10|
|0-5yr  |               1|75+     |DM+HTN    | 10|
|0-5yr  |               1|75+     |HTN only  | 11|
|0-5yr  |               1|65-74   |HTN only  | 12|
|0-5yr  |               1|65-74   |DM+HTN    | 15|
|0-5yr  |               1|55-64   |DM+HTN    | 16|
|0-5yr  |               0|75+     |DM only   | 18|
|0-5yr  |               0|55-64   |DM only   | 23|
...
Table: Unadjusted model — baseline LDL-C effect on MACE (per 1 mg/dL)

|Term         |     HR| 95% CI low| 95% CI high| p-value|
|:------------|------:|----------:|-----------:|-------:|
|baseline_LDL | 0.9953|     0.9943|      0.9963|       0|
...
Table: Schoenfeld residuals — unadjusted model

|Term         | Chi-sq| df| p-value|
|:------------|------:|--:|-------:|
|baseline_LDL |  8.176|  1|  0.0042|
|GLOBAL       |  8.176|  1|  0.0042|
...
::: {.cell-output-display}
![](figures-baseline/ph-unadjusted-1.png){width=2100}
:::
:::
...
Table: Primary baseline model — LDL-C HR per 1 mg/dL

|Term         |     HR| 95% CI low| 95% CI high| p-value|
|:------------|------:|----------:|-----------:|-------:|
|baseline_LDL | 0.9985|     0.9975|      0.9995|  0.0033|
...
Table: Primary baseline model — LDL-C HR per 40 mg/dL

|Term            |     HR| 95% CI low| 95% CI high| p-value|
|:---------------|------:|----------:|-----------:|-------:|
|baseline_LDL_40 | 0.9414|     0.9043|      0.9801|  0.0033|
...
Table: Schoenfeld residuals — primary baseline model

|Term         | Chi-sq| df| p-value|
|:------------|------:|--:|-------:|
|baseline_LDL |  0.668|  1|  0.4137|
|GLOBAL       |  0.668|  1|  0.4137|
...
::: {.cell-output-display}
![](figures-baseline/ph-primary-1.png){width=2100}
:::
:::
...
The proportional hazards assumption holds in the primary baseline model (global p > 0.05).
...
Table: Baseline LDL-C HR per 1 mg/dL across model specifications

|Model                                    |     HR|95% CI          | p-value| N obs| N events|
|:----------------------------------------|------:|:---------------|-------:|-----:|--------:|
|Unadjusted                               | 0.9953|(0.9943–0.9963) |  0.0000| 20007|     2869|
|Stratified, no comorbidity               | 0.9972|(0.9962–0.9982) |  0.0000| 20007|     2869|
|Stratified + cardiomet — PRIMARY         | 0.9985|(0.9975–0.9995) |  0.0033| 20007|     2869|
|Stratified + cardiomet + renal + obesity | 0.9986|(0.9976–0.9996) |  0.0051| 20007|     2869|
|Statin-naive at baseline only            | 0.9984|(0.9974–0.9994) |  0.0020| 19220|     2766|
|Statin-treated at baseline only          | 1.0018|(0.9961–1.0076) |  0.5287|   787|      103|
...
Table: LDL-C effect on MACE — per 40 mg/dL across analytical approaches

|Model                                    |Approach            |     HR|95% CI          | p-value|
|:----------------------------------------|:-------------------|------:|:---------------|-------:|
|Time-varying LDL-C — primary (730d)      |Time-varying (LOCF) | 0.9816|(0.9454–1.0191) |  0.3316|
|Time-varying LDL-C — sensitivity (1825d) |Time-varying (LOCF) | 0.9776|(0.9415–1.015)  |  0.2367|
|Baseline LDL-C — primary                 |Fixed baseline      | 0.9414|(0.9043–0.9801) |  0.0033|
|Baseline LDL-C — statin-naive subgroup   |Fixed baseline      | 0.9415|(0.9037–0.9809) |  0.0040|
...
::: {.cell-output-display}
![](figures-baseline/forest-plot-1.png){width=3000}
:::
:::
