...
```{r naive-note, echo=FALSE, results='asis', cache=FALSE}
# `warning: false` hides the warning above, so surface the empty-pull case visibly.
if (nrow(naive_ad) == 0)
  cat("::: {.callout-warning}\n",
      "**Naive E→AD pull returned no rows** — the only network step here failed (everything else ",
      "on this page reads local CSVs and rendered fine). Likely an **invalid/expired OpenGWAS ",
      "token** or an exhausted allowance — the setup check only tests that a token *string* is ",
      "present, not that it is valid. Verify with `ieugwasr::user()` (a valid token returns your ",
      "account; an invalid one errors), refresh `OPENGWAS_JWT` in `~/.Renviron`, restart R, and ",
      "re-render. The first successful pull is cached to `exposure_ad_naive.csv`.\n",
      ":::\n", sep = "")
```
...
```{r bsh-orientation-check, eval=FALSE}
...
```{r forest, fig.width=8, fig.height=4.5, cache=FALSE}
