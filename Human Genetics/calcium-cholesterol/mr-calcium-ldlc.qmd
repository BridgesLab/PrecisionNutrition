...
From the CAUSE analyses there is qualitative evidence to prefer the causal pathway compared with the shared (pleiotropic) pathways (p=`r summary(calcium.ldlc.cause, ci_size = 0.95)$p`). The estimated causal effect ($\gamma$) is `r summary(calcium.ldlc.cause, ci_size = 0.95)$tab[2,'gamma']` and the residual correlated pleiotropy was minimal after accounting for this causal effect. The $\eta$ = `r summary(calcium.ldlc.cause, ci_size = 0.95)$tab[2,'eta']` is near zero for the causal model but is slightly larger for the sharing model [$\eta$=`r summary(calcium.ldlc.cause, ci_size = 0.95)$tab[1,'eta']`]. To explain this data without a causal effect, CAUSE would require more correlated pleiotropy.  In the absence of a causal effect (sharing model), correlated horizontal pleiotropy would explain `r summary(calcium.ldlc.cause, ci_size = 0.95)$tab[2,'q']`% of the SNPs would require correlated pleiotropy for the causal model, but `r summary(calcium.ldlc.cause, ci_size = 0.95)$tab[1,'q']`% of the SNPs would. 

Alternate explanation with assistance from ChatGPT:

The CAUSE model comparison favored the causal model over both the null and sharing models, although none of the differences reached statistical significance. For example, comparing the sharing vs. causal models yielded a $\Delta$ELPD (Expected Log Pointwise Predictive Density) of `r calcium.ldlc.cause$elpd$delta_elpd[calcium.ldlc.cause$elpd$model1=="sharing"]` with a standard error of `r calcium.ldlc.cause$elpd$se_delta_elpd[calcium.ldlc.cause$elpd$model1=="sharing"]` (z score of = `r calcium.ldlc.cause$elpd$z[calcium.ldlc.cause$elpd$model1=="sharing"]`). The causal model estimated a null effect of calcium on LDL-C (`r summary(calcium.ldlc.cause)$tab[2,"gamma"]`), while the corresponding pleiotropic parameter $\eta$ was centered near zero (`r summary(calcium.ldlc.cause)$tab[2,"eta"]`), suggesting minimal directional pleiotropy. The estimated fraction of variants exhibiting correlated pleiotropy (q) was small under the causal model (`r summary(calcium.ldlc.cause)$tab[2,"q"]`), and lower than under the sharing model (`r summary(calcium.ldlc.cause)$tab[1,"q"]`). 

### Summary of Analyses

```{r calcium-ldlc-mr-summary}
