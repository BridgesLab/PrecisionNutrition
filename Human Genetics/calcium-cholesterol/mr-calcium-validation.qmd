...
The primary result, using the Inverse variance weighted (multiplicative random effects) method shows a `r calcium.control.mr %>% filter(method=="Inverse variance weighted (multiplicative random effects)") %>% pull(b)` $\pm$ `r calcium.control.mr %>% filter(method=="Inverse variance weighted (multiplicative random effects)") %>% pull(se)` SD increase in calcium (MGI-BioVU LabWAS) per 1 SD increase in calcium (UK Biobank).  This is statistically significant with a p-value of `r calcium.control.mr %>% filter(method=="Inverse variance weighted (multiplicative random effects)") %>% pull(pval)`.  All six MR methods (IVW, weighted median, weighted mode, MR-Egger, MR-PRESSO with and without outliers) gave consistent, significant causal estimates, confirming instrument validity and harmonisation.
...
From MR-PRESSO there were `r length(calcium.distortion.test[[1]])` outliers that were removed before the distortion test.  The ratio of the IVW estimate before and after their removal was `r calcium.distortion.test[[2]]`, with a p-value of `r calcium.distortion.test[[3]]`, indicating `r if(calcium.distortion.test[[3]] < 0.05){"significant"}else{"no significant"}` distortion due to outliers.  The global test was significant, showing evidence of average directional horizontal pleiotropy so we should prefer the corrected estimate.  This is consistent with the evidence from the MR-Egger intercept.  This means we should prefer the MR-PRESSO pleiotropy-corrected estimate as the primary result.

```{r calcium-calcium-funnel-plot}
...
From the CAUSE analyses there is significant evidence to prefer the causal pathway compared with the shared (pleiotropic) pathways (p=`r summary(calcium.control.cause, ci_size = 0.95)$p`). The estimated causal effect ($\gamma$) is `r summary(calcium.control.cause, ci_size = 0.95)$tab[2,'gamma']` and the residual correlated pleiotropy was minimal after accounting for this causal effect. The $\eta$ = `r summary(calcium.control.cause, ci_size = 0.95)$tab[2,'eta']` is near zero for the causal model but is substantial for the sharing model [$\eta$=`r summary(calcium.control.cause, ci_size = 0.95)$tab[1,'eta']`]]. In the absence of a causal effect (sharing model), correlated horizontal pleiotropy would explain `r summary(calcium.control.cause, ci_size = 0.95)$tab[1,'q']`% of the genetic correlation between traits. However, the preference for the causal model indicates the observed correlation is primarily driven by the causal pathway rather than shared pleiotropy.

### Leave-one-out Analysis

Using IVW methods

```{r calcium-calcium-mr-loo}
# LOO using IVW
