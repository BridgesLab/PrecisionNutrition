...
# Attach the analysis packages here, in a NON-cached chunk. Loading them inside
# a cached chunk is unsafe: on a cache hit knitr restores the chunk's objects
# but does not re-run library(), so downstream chunks (e.g. mr_singlesnp,
# mr_leaveoneout, mr_presso) would fail to find the functions.
library(TwoSampleMR)
library(ieugwasr)
library(MRPRESSO)
...
# Defined here (not in a cached chunk) so it is always available to the
# cached MR-PRESSO chunks that call it.
...
```{r vitd-replication-comparison}
#| label: vitd-replication-comparison
#| fig-width: 8
#| fig-height: 3.5
...
```{r vitd-replication-agreement}
#| label: vitd-replication-agreement
#| echo: false

# Primary (Revez) IVW-RE beta, computed locally so this chunk does not depend
# on the later vitd-interpretation chunk
vitd.primary.b <- filter(vitd.mr,
                         method == "Inverse variance weighted (multiplicative random effects)") %>%
  pull(b)

vitdbv.ivw <- filter(vitdbv.mr,
                     method == "Inverse variance weighted (multiplicative random effects)")
vitdbv.b <- vitdbv.ivw$b
vitdbv.p <- vitdbv.ivw$pval

vitd.same_sign <- sign(vitd.primary.b) == sign(vitdbv.b)
vitd.replication_phrase <- if (vitd.same_sign) {
  paste0("agrees in direction with the primary Revez 2020 estimate (both ",
         if (vitd.primary.b > 0) "positive" else "negative", ")")
} else {
  "is in the opposite direction to the primary Revez 2020 estimate"
}
```

The MGI-BioVU replication IVW-RE estimate (β = `r round(vitdbv.b, 4)`,
p = `r format.pval(vitdbv.p, digits = 2)`) `r vitd.replication_phrase`. Given
...
```{r vitd-sensitivity-bayes}
#| label: vitd-sensitivity-bayes
...
```{r vitd-interpretation}
#| label: vitd-interpretation
#| echo: false

# Data-driven interpretation of the vitamin D result. The higher-powered
# Revez 2020 GWAS can change the sign/significance relative to the earlier
# MGI-BioVU analysis, so the narrative below is generated from the estimate
# rather than hard-coded.

vitd.ivw <- filter(vitd.mr,
                   method == "Inverse variance weighted (multiplicative random effects)")
vitd.b  <- vitd.ivw$b
vitd.se <- vitd.ivw$se
vitd.p  <- vitd.ivw$pval

vitd.sign_word   <- if (vitd.b > 0) "positive" else "negative"
vitd.supports_h2 <- vitd.b > 0            # H2 predicts a positive cholesterol->VitD effect
vitd.sig         <- vitd.p < 0.05
vitd.consistent  <- if (vitd.supports_h2) {
  "consistent with"
} else {
  "the *opposite* sign from"
}

# MR-PRESSO outlier count (may be unavailable if the test could not run)
vitd.n_outliers <- tryCatch(
  sum(vitd.mrpresso[["MR-PRESSO results"]][["Outlier Test"]][["Pvalue"]] < 0.05,
      na.rm = TRUE),
  error = function(e) NA_integer_
)
vitd.outlier_phrase <- if (is.na(vitd.n_outliers)) {
  "the per-SNP outlier test could not be evaluated"
} else if (vitd.n_outliers == 0) {
  paste0("no individual SNPs met the outlier threshold, indicating diffuse ",
         "rather than concentrated pleiotropy")
} else {
  paste0(vitd.n_outliers, " of ", nrow(vitd.data_steiger),
         " SNPs were flagged as pleiotropic outliers")
}

# Global heterogeneity p-value phrase (MR-PRESSO may have returned NULL)
vitd.global_p <- tryCatch(
  as.numeric(str_remove(
    as.character(vitd.mrpresso[["MR-PRESSO results"]][["Global Test"]][["Pvalue"]]),
    "<")),
  error = function(e) NA_real_
)
vitd.global_phrase <- if (is.na(vitd.global_p)) {
  "MR-PRESSO could not be evaluated for this outcome"
} else {
  paste0("MR-PRESSO returned a global heterogeneity test p = ",
         format.pval(vitd.global_p, digits = 2))
}

# Overall verdict: H2 requires a positive AND statistically significant effect
vitd.meets_threshold <- vitd.supports_h2 && vitd.sig
vitd.verdict_heading <- if (vitd.meets_threshold) {
  "Vitamin D Supports the Threshold for a Causal Mediator"
} else {
  "Vitamin D Does Not Meet the Threshold for a Causal Mediator"
}
vitd.verdict_sentence <- if (vitd.meets_threshold) {
  paste0("vitamin D meets the pre-specified threshold for a causal mediator ",
         "(a positive, statistically significant IVW-RE estimate), supporting ",
         "H2 alongside the bone demineralisation mechanism")
} else {
  paste0("vitamin D does not meet the threshold for a causal mediator, and we ",
         "focus the remainder of this analysis on the bone demineralisation ",
         "mechanism")
}
```

### `r vitd.verdict_heading`

Relative to the heel BMD result, the higher-powered Revez 2020 vitamin D
GWAS gives an IVW-RE point estimate that is `r vitd.sign_word`
(β = `r round(vitd.b, 4)`, SE = `r round(vitd.se, 4)`,
p = `r format.pval(vitd.p, digits = 2)`), which is `r vitd.consistent`
...
`r vitd.global_phrase`, and at NbDistribution = 50,000
`r vitd.outlier_phrase`. The Bayesian
posterior probability for H2 (cholesterol → increased vitamin D) was
`r round(p_h2_true * 100, 1)`%. We therefore conclude that
`r vitd.verdict_sentence`.
