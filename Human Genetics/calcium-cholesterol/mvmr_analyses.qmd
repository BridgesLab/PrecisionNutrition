...
# Standard-error helper reused across the repo
se <- function(x) sd(x, na.rm = TRUE) / sqrt(length(x))

library(tidyverse)
library(knitr)

# University of Michigan palette (matches the rest of the project)
...
```{r helpers-robustness}
#| label: helpers-robustness
...
```{r precompute-constants}
#| label: precompute-constants
#| cache: true
...
```{r drug-target-driver}
#| label: drug-target-driver
...
```{r run-drug-targets-tc}
#| label: run-drug-targets-tc
#| cache: true
...
# Pull the quantities the conclusion actually rests on, straight from the
# results so the narrative below can never drift from the numbers.
get_val <- function(pathway_name, model_name, col, cfg = "UKB primary (LDL-C)") {
  v <- results_summary %>%
    filter(config == cfg, pathway == pathway_name, model == model_name) %>%
    pull(.data[[col]])
  if (length(v) == 0) NA_real_ else v[1]
}

# The decisive leg: the mediator's own effect on the outcome (heel eBMD -> Ca),
# taken from the all-SNP MVMR. If this is null, no BMD-mediated path can carry.
bmd_ca <- mv_res_ivw %>% filter(exposure == GWAS$bmd)

hmgcr_total    <- get_val("HMGCR", "Univariable (total)",    "beta")
hmgcr_total_p  <- get_val("HMGCR", "Univariable (total)",    "pval")
hmgcr_two_pm   <- get_val("HMGCR", "Two-step (product a*b)", "pct_mediated")
allsnp_total   <- get_val("All LDL-C SNPs", "Univariable (total)", "beta")
allsnp_total_p <- get_val("All LDL-C SNPs", "Univariable (total)", "pval")

cat("**Does heel eBMD mediate the cholesterol → serum-calcium effect? No.**\n\n")
cat("- **Mediator → outcome (decisive leg):** heel eBMD → calcium = ",
    signif(bmd_ca$beta, 2), " (p = ", signif(bmd_ca$pval, 2),
    "), indistinguishable from zero. Because the mediator does not move the ",
    "outcome, no BMD-mediated path can carry an appreciable effect — however ",
    "strongly cholesterol moves BMD.\n", sep = "")
cat("- **HMGCR → calcium total effect:** ", signif(hmgcr_total, 3),
    " (p = ", signif(hmgcr_total_p, 2), ") — strong and robust.\n", sep = "")
cat("- **HMGCR mediation via BMD (two-step product, the trustworthy estimate):** ",
    round(hmgcr_two_pm, 1), "% — essentially none.\n", sep = "")
cat("- **HMGCR-specific vs generic LDL:** the HMGCR cis effect (",
    signif(hmgcr_total, 2), ") is ~", round(hmgcr_total / allsnp_total),
    "× the all-LDL-SNP effect (", signif(allsnp_total, 2), ", p = ",
    signif(allsnp_total_p, 2), ", non-significant).\n", sep = "")
```
