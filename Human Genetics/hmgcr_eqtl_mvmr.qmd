...
::: {.callout-important title="Status (2026-07-13): thread CLOSED — total-expression eQTL MR is not viable for HMGCR"}
This analysis is **halted by design**; it is kept for the one solid result and as
a record of the limitation.

**HMGCR is regulated by *splicing*, not total expression.** Its functional
variants are null for total mRNA (`ge`) in GTEx: rs3846662 (exon-13 splice
variant) p = 0.78 (blood) / 0.93 (liver); rs12916 p ≈ 0.46; rs6453133 p ≈ 0.65. A
gene-expression eQTL is therefore blind to the causal mechanism. GTEx per-tissue
power is far too low regardless (best HMGCR `ge` p ≈ 1e-3; kidney n = 73), so the
multi-tissue calcium/kidney/liver screen and colocalization cannot run.

**Kept result:** HMGCR expression (whole blood, eQTLGen) → heel BMD,
β = −0.044, p = 4.4e-5, F = 224 (single cis-eQTL rs6453133, Wald ratio). Correct
sign (↑HMGCR → ↓BMD); whole blood proxies the osteoclast/myeloid lineage, so this
supports the cell-autonomous osteoclast model. **blood → calcium could not be
tested** — rs6453133 is multiallelic and was dropped by the calcium GWAS's
biallelic QC.

**If ever revisited:** use the **HMGCR splice-QTL** (eQTL Catalogue
`leafcutter`/`txrev`, the rs3846662 exon-13 event) — the functionally correct,
strong, biallelic exposure — or eQTLGen-full blood `ge`. Not critical: the
drug-target cis-MR already provides a valid HMGCR instrument. See `ANALYSIS.md` §7.
:::
...
if (nrow(eqtl_mr_grid) == 0) {
  cat("No tissue produced a usable HMGCR instrument — fill in real eQTL accessions ",
      "in the catalogue (or drop local files) and re-render.\n")
} else {
  ran   <- eqtl_mr_grid %>% filter(note == "ok" | is.na(note) | note == "")
  drop  <- eqtl_mr_grid %>% filter(!(note == "ok" | is.na(note) | note == ""))
  bmd   <- ran %>% filter(outcome == "Heel BMD", pval < 0.05)
  ca    <- ran %>% filter(outcome == "Serum calcium", pval < 0.05)
  cadrop <- drop %>% filter(outcome == "Serum calcium")
  cat("- **HMGCR expression → BMD** is significant in: ",
      ifelse(nrow(bmd) > 0, paste0(bmd$tissue, " (β ", signif(bmd$beta, 2), ")", collapse = ", "),
             "none of the tissues that ran"),
      ". Inverse (↓BMD) is the predicted sign; a whole-blood/monocyte signal supports the cell-autonomous osteoclast model.\n", sep = "")
  if (nrow(cadrop) > 0) {
    cat("- **HMGCR expression → calcium: could not be tested** for ",
        paste(cadrop$tissue, collapse = ", "), " — ", cadrop$note[1],
        ". This is a COVERAGE issue (single cis-eQTL not in the calcium GWAS), **not** evidence of no effect. ",
        "Add proxy SNPs or a second cis instrument, or test the liver/kidney arms.\n", sep = "")
  } else {
    cat("- **HMGCR expression → calcium** is significant in: ",
        ifelse(nrow(ca) > 0, paste(ca$tissue, collapse = ", "), "none of the tissues that ran"),
        ". A kidney signal would support the renal hypothesis; liver the systemic route.\n", sep = "")
  }
  cat("\n**Read tissue results as suggestive** (cis-eQTL sharing limits attribution); ",
      "the coloc H4 above is the defensible claim that the effect is HMGCR-mediated. ",
      "Signs should match the cholesterol cis-MR: ↑HMGCR → ↑calcium, ↓BMD.\n", sep = "")
}
```
