...
The primary result, using the inverse variance weighted method shows a `r calcium.ldlc.mr %>% filter(method=="Inverse variance weighted") %>% pull(b)` $\pm$ `r calcium.ldlc.mr %>% filter(method=="Inverse variance weighted") %>% pull(se)` SD increase in LDL cholesterol (MGI-BioVU LabWAS) per 1 SD increase in calcium (UK Biobank).  This is statistically significant with a p-value of `r calcium.ldlc.mr %>% filter(method=="Inverse variance weighted") %>% pull(pval)`.  All four MR methods (IVW, weighted mode, weighted median, MR-Egger) gave consistent, **non-significant** causal estimates, supporting the hypothesis of no causal effect of calcium on LDL-C.  
