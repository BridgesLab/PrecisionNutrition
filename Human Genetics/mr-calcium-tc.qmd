...
This presumes the sample sizes was `r as.character(samplesize.outcome.tc)` from Table 1 of https://doi.org/10.1371/journal.pgen.1009077.

Loaded in the instruments for calcium from UK Biobank from the datafile `r instruments.calcium.file` and the GWAS summary statistics for calcium from the datafile `r gwas.tc.file`.

```{r calcium-tc-harmonizing-snps}
library(TwoSampleMR)
...
- There were `r length(intersect(instruments.calcium$SNP, gwas.tc$SNP))` SNPs in common between the exposure and outcome datasets.
...
The primary result, using the inverse variance weighted (multiplicative random effects) method shows a `r calcium.tc.mr %>% filter(method=="Inverse variance weighted (multiplicative random effects)") %>% pull(b)` $\pm$ `r calcium.tc.mr %>% filter(method=="Inverse variance weighted (multiplicative random effects)") %>% pull(se)` SD increase in total cholesterol (MGI-BioVU LabWAS) per 1 SD increase in calcium (UK Biobank).  This is statistically significant with a p-value of `r calcium.tc.mr %>% filter(method=="Inverse variance weighted (multiplicative random effects)") %>% pull(pval)`.  All four MR methods (IVW, weighted median, weighted mode, MR-Egger) gave consistent, **non-significant** causal estimates, confirming instrument validity and harmonisation.

### MR-Egger Intercept

```{r calcium-tc-mr-pleiotropy}
...
From the CAUSE analyses there is qualitative evidence to prefer the causal pathway compared with the shared (pleiotropic) pathways (p=`r summary(calcium.tc.cause, ci_size = 0.95)$p`). The estimated causal effect ($\gamma$) is `r summary(calcium.tc.cause, ci_size = 0.95)$tab[2,'gamma']` and the residual correlated pleiotropy was minimal after accounting for this causal effect. The $\eta$ = `r summary(calcium.tc.cause, ci_size = 0.95)$tab[2,'eta']` is near zero for the causal model but is slightly larger for the sharing model [$\eta$=`r summary(calcium.tc.cause, ci_size = 0.95)$tab[1,'eta']`]. To explain this data without a causal effect, CAUSE would require more correlated pleiotropy.  In the absence of a causal effect (sharing model), correlated horizontal pleiotropy would explain `r summary(calcium.tc.cause, ci_size = 0.95)$tab[2,'q']`% of the SNPs would require correlated pleiotropy for the causal model, but `r summary(calcium.tc.cause, ci_size = 0.95)$tab[1,'q']`% of the SNPs would. Overall, CAUSE provided weak but consistent evidence favoring a causal model over correlated pleiotropy.

Alternate explanation with assistance from ChatGPT:

The CAUSE model comparison favored the causal model over both the null and sharing models, although none of the differences reached statistical significance. For example, comparing the sharing vs. causal models yielded a $\Delta$ELPD (Expected Log Pointwise Predictive Density) of `r calcium.tc.cause$elpd$delta_elpd[calcium.tc.cause$elpd$model1=="sharing"]` with a standard error of `r calcium.tc.cause$elpd$se_delta_elpd[calcium.tc.cause$elpd$model1=="sharing"]` (z score of = `r calcium.tc.cause$elpd$z[calcium.tc.cause$elpd$model1=="sharing"]`). The causal model estimated a positive effect of LDL-C on calcium (`r summary(calcium.tc.cause)$tab[2,"gamma"]`), while the corresponding pleiotropic parameter $\eta$ was centered near zero (`r summary(calcium.tc.cause)$tab[2,"eta"]`), suggesting minimal directional pleiotropy. The estimated fraction of variants exhibiting correlated pleiotropy (q) was small under the causal model (`r summary(calcium.tc.cause)$tab[2,"q"]`), and lower than under the sharing model (`r summary(calcium.tc.cause)$tab[1,"q"]`). Together, these results indicate that correlated pleiotropy does not adequately explain the SNP–trait associations and that the CAUSE analysis is most consistent with a causal effect.

### Summary of Analyses

```{r calcium-tc-mr-summary}
