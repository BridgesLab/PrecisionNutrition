...
The primary result, using the inverse variance weighted method shows a `r calcium.tc.mr %>% filter(method=="Inverse variance weighted") %>% pull(b)` $\pm$ `r calcium.tc.mr %>% filter(method=="Inverse variance weighted") %>% pull(se)` SD increase in total cholesterol (MGI-BioVU LabWAS) per 1 SD increase in calcium (UK Biobank).  This is statistically significant with a p-value of `r calcium.tc.mr %>% filter(method=="Inverse variance weighted") %>% pull(pval)`.  All four MR methods (IVW, weighted median, weighted mode, MR-Egger) gave consistent, **non-significant** causal estimates, confirming instrument validity and harmonisation.
