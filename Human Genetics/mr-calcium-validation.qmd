...
From the CAUSE analyses there is significant evidence to prefer the causal pathway compared with the shared (pleiotropic) pathways (p=`r summary(calcium.control.cause, ci_size = 0.95)$p`). The estimated causal effect ($\gamma$) is `r summary(calcium.control.cause, ci_size = 0.95)$tab[2,'gamma']` and the residual correlated pleiotropy was minimal after accounting for this causal effect. The $\eta$ = `r summary(calcium.control.cause, ci_size = 0.95)$tab[2,'eta']` is near zero for the causal model but is substantial for the sharing model [$\eta$=`r summary(calcium.control.cause, ci_size = 0.95)$tab[2,'eta']`]]. In the absence of a causal effect (sharing model), correlated horizontal pleiotropy would explain `r summary(calcium.control.cause, ci_size = 0.95)$tab[1,'q']`% of the genetic correlation between traits. However, the preference for the causal model indicates the observed correlation is primarily driven by the causal pathway rather than shared pleiotropy.
