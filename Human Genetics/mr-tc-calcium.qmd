...
The primary result, using the inverse variance weighted method shows a `r tc.control.mr %>% filter(method=="Inverse variance weighted") %>% pull(b)` $\pm$ `r tc.control.mr %>% filter(method=="Inverse variance weighted") %>% pull(se)` SD increase in calcium (MGI-BioVU LabWAS) per 1 SD increase in total cholesterol (UK Biobank).  This is statistically significant with a p-value of `r tc.control.mr %>% filter(method=="Inverse variance weighted") %>% pull(pval)`.  Three of the four MR methods (IVW, weighted median, weighted mode) gave consistent, significant causal estimates, supporting the hypothesis that total cholesterol may impact serum calcium levels in a positive direction.  The MR-Egger estimate was not statistically significant, but this method is known to have low power.
