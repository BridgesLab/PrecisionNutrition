...
The primary result, using the inverse variance weighted (multiplicative random effects) method shows a `r tc.calcium.mr %>% filter(method=="Inverse variance weighted (multiplicative random effects)") %>% pull(b)` $\pm$ `r tc.calcium.mr %>% filter(method=="Inverse variance weighted (multiplicative random effects)") %>% pull(se)` mg/dL increase in calcium (MGI-BioVU LabWAS) per 1 SD increase in total cholesterol (UK Biobank).  This is statistically significant with a p-value of `r tc.calcium.mr %>% filter(method=="Inverse variance weighted (multiplicative random effects)") %>% pull(pval)`.  Three of the four MR methods (IVW, weighted median, weighted mode) gave consistent, significant causal estimates, supporting the hypothesis that total cholesterol may impact serum calcium levels in a positive direction.  The MR-Egger estimate was not statistically significant, but this method is known to have low power.
...
From the CAUSE analyses there is qualitative evidence to prefer the causal pathway compared with the shared (pleiotropic) pathways (p=`r summary(tc.calcium.cause, ci_size = 0.95)$p`). The estimated causal effect ($\gamma$) is `r summary(tc.calcium.cause, ci_size = 0.95)$tab[2,'gamma']` and the residual correlated pleiotropy was minimal after accounting for this causal effect. The $\eta$ = `r summary(tc.calcium.cause, ci_size = 0.95)$tab[2,'eta']` is near zero for the causal model but is slightly larger for the sharing model [$\eta$=`r summary(tc.calcium.cause, ci_size = 0.95)$tab[1,'eta']`]. To explain this data without a causal effect, CAUSE would require more correlated pleiotropy.  In the absence of a causal effect (sharing model), correlated horizontal pleiotropy would explain `r summary(tc.calcium.cause, ci_size = 0.95)$tab[2,'q']`% of the SNPs would require correlated pleiotropy for the causal model, but `r summary(tc.calcium.cause, ci_size = 0.95)$tab[1,'q']`% of the SNPs would. Overall, CAUSE provided weak but consistent evidence favoring a causal model over correlated pleiotropy.

Alternate explanation with assistance from ChatGPT:

The CAUSE model comparison favored the causal model over both the null and sharing models, although none of the differences reached statistical significance. For example, comparing the sharing vs. causal models yielded a $\Delta$ELPD (Expected Log Pointwise Predictive Density) of `r tc.calcium.cause$elpd$delta_elpd[tc.calcium.cause$elpd$model1=="sharing"]` with a standard error of `r tc.calcium.cause$elpd$se_delta_elpd[tc.calcium.cause$elpd$model1=="sharing"]` (z score of = `r tc.calcium.cause$elpd$z[tc.calcium.cause$elpd$model1=="sharing"]`). The causal model estimated a positive effect of cholesterol on calcium (`r summary(tc.calcium.cause)$tab[2,"gamma"]`), while the corresponding pleiotropic parameter $\eta$ was centered near zero (`r summary(tc.calcium.cause)$tab[2,"eta"]`), suggesting minimal directional pleiotropy. The estimated fraction of variants exhibiting correlated pleiotropy (q) was small under the causal model (`r summary(tc.calcium.cause)$tab[2,"q"]`), and lower than under the sharing model (`r summary(tc.calcium.cause)$tab[1,"q"]`). Together, these results indicate that correlated pleiotropy does not adequately explain the SNP–trait associations and that the CAUSE analysis is most consistent with a causal effect.

### Summary of Analyses

```{r tc-calcium-mr-summary}
