...
```{r bootstrapped-regression-tree}
library(rpart)
library(dplyr)
library(rpart.plot)
...
```{r bayesian-calcium-cholesterol}
...
```{r calcium-chol-prior-predictive}
...
The effective sample size was >`r tidy(chol.ca.brms, ess = TRUE)$ess |> min()` and Rhat ranged between `r format(rhat(chol.ca.brms), nsmall = 5) |> min()` and `r format(rhat(chol.ca.brms), nsmall = 5) |> max()`, indicating that the model converged well.

### Posterior Prediction Checks

```{r calcium-chol-posterior-predictive}
...
```{r calcium-chol-posterior-summaries}
plot(chol.ca.brms)
